...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    n_train_rgs = max(1, int(total_rgs * (1 - valid_ratio)))
    n_valid_rgs = total_rgs - n_train_rgs
    ...
    # Single-RG fallback: when valid gets 0 RGs, let valid share the same RGs as train
    # so evaluation has data (e.g. demo_1000.parquet).
    valid_start_rg = n_train_rgs
    if n_valid_rgs == 0:
        n_valid_rgs = n_train_rgs
        valid_start_rg = 0
